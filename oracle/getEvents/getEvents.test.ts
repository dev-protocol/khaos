...
test('event information is coming back.', async (t) => {
	const web3 = new Web3EventMock('dummy-url')
	const events = await getEvents(web3 as any, '', 0, 100)
	t.is(events.length, 2)
	t.is(events[0].returnValues._data.key1, 'value1')
	t.is(events[1].returnValues._data.key2, 'value2')
})
